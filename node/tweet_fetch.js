...
// code without any repeat tweet new
...
// ✅ Helper: Create Rettiwt instance with random delay and random key
...
async function fetchTweets(topic, totalCount) {
    try {
        ...
        const allTweets = [];
        const seenIds = new Set(); // Track tweet IDs to prevent duplicates
        ...
        for (const filter of filters) {
            const rettiwt = createRandomRettiwt(); // 🔁 New instance per filter
            ...
            while (hasMore && allTweets.length < totalCount) {
                ...
                console.log(`📡 Fetching ${batchSize} tweets...`);
                ...
                for (const tweet of tweets.list) {
                    ...
                    if (!tweet.fullText || tweet.fullText.length <= 20) continue;
                    if (franc(tweet.fullText) !== 'eng') continue;

                    seenIds.add(tweet.id);
                    newTweets.push({ text: tweet.fullText });
                }

                allTweets.push(...newTweets);
                console.log(`✅ Added ${newTweets.length} new tweets (total: ${allTweets.length}/${totalCount})`);
        ...
        if (allTweets.length > 0) {
            ...
            console.log(`🎉 Successfully saved ${allTweets.length} unique English tweets`);
        } else {
...
// ✅ Robust CLI Input Handling (borrowed from second code)
const rawInput = process.argv.slice(2).join(" ").trim();

if (!rawInput) {
    console.error("❌ Error: Please provide a topic to search for.");
    process.exit(1);
}

const topic = rawInput.toLowerCase();
const count = 20;
